...
export default function CartPage() {
  ...
  return (
    <div className="min-h-screen pb-32">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="space-y-3">
          {items.map((item) => (
            <div
              key={item.id}
              className="bg-white rounded-2xl border border-brand-100/60 p-4 animate-fade-in"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-1">
                    <span className="text-xs bg-brand-50 text-brand-600 px-2 py-0.5 rounded-lg">
              ...
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-50">
                <div className="flex items-center gap-2">
                  ...
                  <span className="w-6 text-center font-bold text-sm text-brand-900">
                    {item.quantity}
                  </span>
                  <button
                    onClick={() => updateQuantity(item.id, item.quantity + 1)}
                    className="w-7 h-7 rounded-lg bg-brand-100 hover:bg-brand-200 flex items-center justify-center transition-colors"
                  >
      ...
      <div className="fixed bottom-0 inset-x-0 glass border-t border-brand-100 p-4 z-40">
        <div className="max-w-2xl mx-auto">
          ...
          <Link
            href="/checkout"
            className="block w-full bg-brand-700 hover:bg-brand-800 text-white text-center py-4 rounded-2xl font-semibold shadow-lg shadow-brand-700/25 transition-all active:scale-[0.98]"
          >
            Proceed to Checkout
          </Link>
        </div>
